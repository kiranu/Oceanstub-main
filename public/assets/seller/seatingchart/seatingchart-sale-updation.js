...
stage.on('mouseover', function(e) {
    ...
    if (e.target.attrs.type == "rowindividualseats" && e.target.attrs.category == "seats") {
        if(e.target.attrs.PriceLevel != "novalue" && e.target.attrs.PriceLevel != undefined ) {
            $('.individual-seat-details-for-table-with-seats').css('top', e.evt.pageY - 150 + 'px');
            $('.individual-seat-details-for-table-with-seats').css('left', e.evt.pageX  - 150 + 'px');
    ...
    if(e.target.attrs.type == "rowindividualseats") {
        ...
        if(e.target.attrs.ticketAdded == true) {
            if(e.target.attrs.ishide != true) {
                ...
                $('.idorclass').val("name");
                $('.idorclassifo').val(e.target.attrs.name);
                ...
                $('.ticketid').val(e.target.attrs.ticketid);
        ...
        for(var l=0;l<rowseats[0].parent.children.length;l++){
            if(rowseats[0].parent.children[l].attrs.type == "rowindividualseats") {
                if(rowseats[0].parent.children[l].attrs.ticketAdded == true) {
                    ...
                    //console.log(rowseats[0].parent.children[l].attrs.ticketid);
                    ...
                    if(b == 1) {
                        ...
                        //console.log(rowseats[0].parent.children[l].attrs.ticketid);
                        $('.ticketid').val(rowseats[0].parent.children[l].attrs.ticketid);
                        b++;
                        $(".variablepricecontainer").empty();
                ...
                if(seats[0].parent.children[n].attrs.type == "rowindividualseats") {
                    if(seats[0].parent.children[n].attrs.ticketAdded == true) {
                        if(seats[0].parent.children[n].attrs.ishide != true) {
                            ...
                            names.push(seats[0].parent.children[n].attrs.name);
                        }
                    }
                }
            }
        ...
        var sectiondetails = stage.find("#"+e.target.attrs.selectedsectionid);
            ...
            if(e.target.attrs.ticketAdded == true) {
                if(e.target.attrs.ishide != true) {
                    $("#ticketdetailsbigpopup").dialog('open');
                    $('.idorclass').val("id");
                    $('.idorclassifo').val(e.target.attrs.id);
                    $(".ind-row-name").val('');
                    $(".ind-sec-seat-no").val(e.target.attrs.seatno);
                    $(".ind-sec-seat-price").val(e.target.attrs.PriceLevel);
            ...
            if((avgpriceval/count) == firstpriceval) {
                ...
                for(var m=0;m<e.target.parent.children[0].children.length;m++) {
                    if(e.target.parent.children[0].children[m].attrs.ticketAdded == true) {
                        ...
                        $('.ticketid').val(e.target.parent.children[0].children[m].attrs.ticketid);
                        ids.push(e.target.parent.children[0].children[m].attrs.id);
            ...
            if(e.target.attrs.ticketAdded != false && e.target.attrs.ticketAdded != undefined) {
                $(".variablepricecontainer").empty();
                console.log(e.target.attrs);
                $("#ticketdetailsbigpopup").dialog('open');
                $(".rowride").css('display','none');
...
function updateseatingchart() {
    ...
    var gaselectedseats = [];
    var gaselected;
    $('.idorclasscs').each(function(){
        types.push(this.value);
    });
    $('.idorclassval').each(function(){
        var cc = this.value;
        typeval.push(cc);
    });
    $('.gaselectedseats').each(function(){
        var cc = this.value;
        gaselectedseats.push(cc);
    });
    var d = 0;
    for(var i = 0; i< types.length;i++) {
        type = types[i];
        typevalsingle = typeval[i];
        gaselected = gaselectedseats[d];
        typevalsingle = typevalsingle.split(',');
        console.log(typevalsingle);
        typevalsingle[0] = typevalsingle[0].replace(/\"/g, "");
        shape = stage.find("#"+typevalsingle[0]);
        if(type == "id") {
            if(shape[0].attrs.type == "other") {
                shape = stage.find("#"+typevalsingle);
                shape.setAttr('ticketAdded',false);
                shape.setAttr('sold',"yes");
                layer.draw();
            }
            else if(shape[0].attrs.type == "tablewithseats")  {
                if(typevalsingle.length == 1 ) {
                    console.log(typevalsingle);
                    shape = stage.find("#"+typevalsingle);
                    console.log(shape);
                    shape.setAttr('ticketAdded',false);
                ...
                else if (typevalsingle.length > 1 ) {
                    for(m=0;m<typevalsingle.length;m++) {
                        var mn = typevalsingle[m].replace(/\"/g, "");
                        shape = stage.find("#"+mn);
                        console.log(shape);
                        shape.setAttr('ticketAdded',false);
                        shape.setAttr('sold',"yes");
            ...
            else if (shape[0].attrs.ganame != undefined) {
                shape = stage.find("#"+typevalsingle);
                var selectedga = gaselectedseats[d];
                var updatedcapacity = shape[1].attrs.maxgacapacity -  selectedga;
                if(updatedcapacity > 0 ) {
                    shape.setAttr('maxgacapacity',updatedcapacity);
                    shape.setAttr('ticketCount',updatedcapacity);
                }
                else if (updatedcapacity < 0) {
                    shape.setAttr('ticketAdded',false);
                    shape.setAttr('maxgacapacity',0);
                    shape.setAttr('ticketCount',0);
                    shape.setAttr('sold',"yes");
                }
                layer.draw();
                d = d + 1;
            }
            else if (shape[0].attrs.type == "tablewithoutseats") {
                shape = stage.find("#"+typevalsingle);
                console.log(shape);
                var selectedga = gaselectedseats[d];
                var updatedcapacity = shape[1].attrs.Toseatfortablewithoutchair -  selectedga;
                if(updatedcapacity > 0 ) {
                    shape.setAttr('maxgacapacity',updatedcapacity);
                    shape.setAttr('ticketCount',updatedcapacity);
                    shape.setAttr('Toseatfortablewithoutchair',updatedcapacity);
                    layer.draw();
                }
                else if (updatedcapacity < 0) {
                    shape.setAttr('ticketAdded',false);
                    shape.setAttr('maxgacapacity',0);
                    shape.setAttr('ticketCount',0);
                    shape.setAttr('Toseatfortablewithoutchair',0);
                    shape.setAttr('sold',"yes");
                    layer.draw();
                }
                d = d + 1;
