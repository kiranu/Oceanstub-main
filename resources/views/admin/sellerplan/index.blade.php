        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              ...
              {{-- <a href="{{url('admin/seller_plan/create')}}" style="max-width: 150px; float:right" class="btn btn-block btn-success">Add Seller Plan</a> --}}
              <a href="{{url('admin/seller_plan_details')}}" style="max-width: 150px; float:right" class="btn btn-block btn-success">Plan Details</a>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
            ...
            @foreach ($seller_plans as $seller_plan)
                <tr>
                  <td><a href="{{url('admin/seller_plan/'.$seller_plan->id)}}">{{$seller_plan->id}}</a></td>
                  <td>{{$seller_plan->name}}</td>
                  <td>{{$seller_plan->price}}</td>
                  <td>{{$seller_plan->validity}}</td>
                  ...
                  <td><a title="Edit seller_plan" href="{{url('admin/seller_plan/'.$seller_plan->id.'/edit')}}"><i class="fas fa-edit"></i></a>
                  {{-- <a title="Delete Plan" class="confirmDelete" name="seller_plan " href="{{url('admin/seller_plan/delete/'.$seller_plan->id)}}"><i class="fas fa-trash"></i></a> --}}
                  </td>
...
$(function() {
    ...
    $(".UpdateSellerPlanStatus").on("click", function(){
        ...
        $.ajax({
            type:'post',
            url:"{{route('admin.update_seller_plan_status')}}",
