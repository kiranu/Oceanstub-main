...
<input type="hidden" name="event_id" id="event_id" value="{{$event_data->id}}">
              <div class="form-group row2">
                ...
                <div class="col-sm-4">
                  ...
                  name="first_title" 
                  value="{{$event_data->first_title}}" id="inputPassword3" placeholder="">
              ...
              <div class="form-group row1">
                ...
                <div class="col-sm-4">
                  <input type="text" class="form-control" id="second_title" name="second_title" id="inputPassword3"
                  value="@isset($event_data->second_title){{$event_data->second_title }}@endisset" 
                  placeholder="">
                </div>
              </div>
              <div class="form-group row2">
                ...
                name="currency" style="width: 32%;margin-left: 8px;">
                
                  <option value="inr" {{$event_data->currency == "inr" ? 'selected' : '' }} >INR</option>
                  <option value="cad" {{$event_data->currency == "cad" ? 'selected' : '' }} >CAD</option>
                  <option value="usd" {{$event_data->currency == "usd" ? 'selected' : '' }}>USD</option>
                  <option value="eur" {{$event_data->currency == "eur" ? 'selected' : '' }}>EUR</option>
                  <option value="gbp" {{$event_data->currency == "gbp" ? 'selected' : '' }}>GBP</option>
                  <option value="aud" {{$event_data->currency == "aud" ? 'selected' : '' }}>AUD</option>
                  <option value="czk" {{$event_data->currency == "czk" ? 'selected' : '' }}>CZK</option>
                </select>
              ...
              <input type="checkbox" id="ctl02_cIsOnline"

            value="on" name="online" onchange="showHide(this.checked)"
            @isset($event_data->event_types->online)
            {{($event_data->event_types->online == "on" ? 'checked' : '')}}@endisset

             >
             <span class="text-danger error-text online_error"></span>
        ...
        <input type="radio" name="cIsStream" value="false"onchange="hide(this.checked)"
          @isset($event_data->event_types->streaming)
          {{$event_data->event_types->streaming == 'false' ? 'checked' : ''}}@endisset>

        <span class="checkmark" style="margin-left: 5px;" >       
          </span>I will take care of streaming and ticket validation myself</span>  
          <span class="cRadioV2" style="display: block;">

        <input type="radio" name="cIsStream" value="yes" 
          onchange="showHidetwo(this.checked)"
          @isset($event_data->event_types->streaming)
        {{$event_data->event_types->streaming == 'true' ? 'checked' : ''}}@endisset >
                      ...
                      <span class="text-danger error-text cIsStream_error"></span>
                  </div>
                </div>
                <br>
                <div class="cStreamOptions" style="display: none;padding-left:20px">
                  <div class="row">
                    ...
                    <textarea name="StreamEmbedCode" 
                    value="@isset($event_data->event_types->embed_code){{$event_data->event_types->embed_code}}@endisset"
                    rows="5" cols="20" 
                    id="ctl02_cStreamEmbedCode" class="editText" 
                    data-validation-required="1" 
                    style="width:100%;">@isset($event_data->event_types->embed_code){{$event_data->event_types->embed_code}}@endisset</textarea>
                    <span class="text-danger error-text StreamEmbedCode_error"></span>
                  ...
                  <div class="row cAllowReentry" style="text-align:left;">
                    <span class="cCheckboxV2">
                      ...
                      @isset($event_data->event_types->allow_reentry){{  ($event_data->event_types->allow_reentry == "yes" ? ' checked' : '') }} @endisset>
                  ...
                  <div class="row">
                    ...
                    <select name="IsOnDemand" id="ctl02_cIsOnDemand" class="cIsOnDemand" 
                    value="@if(isset($event_data->event_types->on_demand)){{$event_data->event_types->on_demand}}@endif">
                      <option  value="false" 
                      {{$event_data->event_types->on_demand == "false" ? 'selected' : '' }}>Live</option>
                      <option value="true"
                      {{$event_data->event_types->on_demand == "true" ? 'selected' : '' }}
                      >On-demand</option>
                    </select>
           ...
           <!--        <div class="row cOnDemandMinutesContainer" style="margin-top: 10px; display: none;">
              ...
              <div class="cInPersonOptions"style="padding-left:20px;  margin-top: 10px;">
                ...
                <div class="cAssignedSeats radio-group-data-Validation-Required error" data-validation-required="6" aria-describedby="cValMsg574298 " aria-invalid="true">
                  <div class="cAssignedSeats radio-group-data-Validation-Required" data-validation-required="6" aria-describedby="cValMsg574298 " aria-invalid="true">
                    ...
                    @isset($event_data->event_types->seating_type){{($event_data->event_types->seating_type == "0" ? ' checked' : '') }} @endisset >

                    <span class="checkmark" style="margin-left: 5px;"></span>General Admission (first-come, first-serve) or Standing</span>  
                    <span class="cRadioV2" style="display:block;">

                      <input type="radio" name="seating" value="1" class=""
                      @isset($event_data->event_types->seating_type){{($event_data->event_types->seating_type == "1" ? ' checked' : '') }} @endisset>
                      <span class="checkmark" style="margin-left: 5px;">
              ...
              <div class="row2 col-md-6"style="padding-left:20px;display:inline-block">
                ...
                value="@if(isset($event_data->event_types->event_type)){{$event_data->event_types->event_type}}@endif" 
                multiple="multiple" id="type" 
                class="cEventType form-control">
                @if(isset($event_data->event_types->event_type))
                <option selected="" value="{{$event_data->event_types->event_type}}">{{$event_data->event_types->event_type}}</option>
              ...
              <div class="row2 col-md-6 "style="padding-left:20px;display: inline-block;" >
                ...
                value="
                @if(isset($event_data->event_types->event_genre)){{$event_data->event_types->event_genre}}@endif"
                 multiple="multiple" id="genre" class="cEventGenre form-control">
                 @if(isset($event_data->event_types->event_genre))<option selected="" value="{{$event_data->event_types->event_genre}}">{{$event_data->event_types->event_genre}}
                 </option>
                  @endif"
                ...
                <span class="text-danger error-text event_genre_error"></span>
                <br>
                <p class="hint"style="font-size:14px">Hold down the Ctrl (windows) or Command (Mac) button to select multiple options.</p>
              </div>
              </div>
  ...
  <div class=" cVenueSelection cWizStep3" data-wizstep-name="venue saveIfEdit"style="padding-left:20px">
                ...
                <div class="cVenueSelectionContenttwo" >
                  ...
                  <select class="cMyVenues error js-example-basic-multiple-venue form-control" 
                  name="venue"
               ...
               @foreach($venues as $venue)
                ...
                {{(old('venue_list',isset($venue_sch->venue_list) ? $venue_sch->venue_list :null) == $venue->id) ? 'selected' : ''}}>{{$venue->name}}</option>
               @endforeach
...
<div class=" cNonSchedulerRow cWizStep4" style="padding-left:20px">
  ...
  type="date"
   ...
   value="@if(isset($venue_sch->start_date)){{date('Y-m-d', strtotime($venue_sch->start_date))}}@endif" 
    id="ctl02_cDateView" 
    class="editText cDatePicker cDateView hasDatepicker valid eventstart form-control" 
    style="width:160px;display:inline-block;">
    <span class="text-danger error-text start_date_error"></span>

	<!-- <label for="ctl02_cTimeView" style="display:none;">Event Start Time:</label> -->
<div class="fromat_time_div">
  <input class="fromat_time" name="start_time" type="number" min="0" max="23" 
  value="@if(isset($venue_sch->start_time)){{date('H', strtotime($venue_sch->start_time))}}@endif" placeholder="23">
  
 <span style="margin-top:4px;">:</span>
  <input class="fromat_time" name="start_time_min" type="number" min="0" max="59" 
  value="@if(isset($venue_sch->start_time)){{date('i', strtotime($venue_sch->start_time))}}@endif" placeholder="00">
</div>

	<!-- <input name="start_time" type="time" 

  value="@if(isset($venue_sch->start_time)){{date('H:i:s', strtotime($venue_sch->start_time))}}@endif" 

   id="ctl02_cTimeView" 
   class="editText cTimePicker cTimeView hasTimepicker valid form-control"  
   style="width:160px;display:inline-block;"> -->
...
<div class="form-group row" style="margin-left:9.5px;">
     ...
     <div class="row cWizStep4" style="margin-left:20px;">
                ...
                <input id="show_date" type="checkbox" value="1"

                name="show_date"

 @isset($venue_sch->show_date){{  ($venue_sch->show_date == "1" ? ' checked' : '') }} @endisset
                >
              ...
              <div class="row2cWizStep4" style="margin-left:20px;">
                ...
                <input id="show_time" type="checkbox" value="1" name="show_time"
                 @isset($venue_sch->show_time){{($venue_sch->show_time == "1" ? ' checked' : '')}} @endisset
                >
  ...
  <input type="number" name="days" id="days" value="{{$venue_sch->days}}" maxlength="4" class="cDurationDays valid form-control" data-validation-avoid-success-tick="1" data-validation-number="uint" style="width:60px;display:inline-block;" min="0"
  value="">
  <span class="text-danger error-text days_error"></span>

                days</span> <span class="valid">
                <input type="number" name="hours" id="hours" value="{{$venue_sch->hours}}" maxlength="4" class="cDurationHours valid form-control" data-validation-avoid-success-tick="1" data-validation-number="uint" style="width:60px;display:inline-block;"min="0"value=""> hours</span> <span class="valid"
                >
                <input type="number" name="mins" id="mins" value="{{$venue_sch->minuts}}"  maxlength="4" class="cDurationMins valid form-control" data-validation-avoid-success-tick="1" data-validation-number="uint" style="width:60px;display:inline-block;"min="0"value="">mins</span>
              </div>
              ...
              <div class=" cAdv "style="padding-left:10px" >
                <span for="ctl02_cIsStartDateRelative" id="ctl02_Label10" 
                class="editLabel" style="display:inline-block;">Sale for the event starts:</span><br>
 ...
 <span class="text-danger error-text SaleStart_error"></span>
                <span class="cIsStartDateRelative0" style="display:inline-block;width:60% ">

                <input name="sales_start_date" type="date" 
                 value="{{date('Y-m-d', strtotime($event_data->event_sales->start_date))}}" maxlength="10" 
                id="sales_start_date"
                  class="cDatePicker cStartDateView hasDatepicker valid salestartevent form-control" data-validation-required="1" 
                  data-validation-avoid-success-tick="1" 
                  style="display:inline-block;width:30% ">
 
                <input name="sales_start_time" type="time"  
                value="{{date('H:i:s', strtotime($event_data->event_sales->start_time))}}" maxlength="10" id="sales_start_time"class="cTimePicker hasTimepicker valid form-control" data-validation-required="1" data-validation-avoid-success-tick="1" 
                style="width:30% ;display:inline-block;">

                </span>
                <br>
<span class="text-danger error-text sales_start_date_error"></span>
                ...
                name="StartDaysBefor" 
                 maxlength="4"
                 class="cDurationDays form-control" 
                 value="0" 
                  style="width:69px;display:inline-block;"min="0"> days</span>


                <span>
                  <input type="number" 
                  name="StartHoursBefor" 
                  value="0" 
                  maxlength="4" class="cDurationHours form-control"  style="width:69px;display:inline-block;"min="0">
                hours</span>
                ...
                <span> 
                  ...
                  value="0" 
                  name="StartMinsBefor" 
                   maxlength="4" class="cDurationMins form-control" style="width:69px;display:inline-block;"min="0">
                mins</span>
     ...
     <div class=" cAdv" style="padding-left:10px">
                ...
                <select style="width:30% ;display:inline-block;"
                 name="SaleEnd" id="sales_end_at" class="cIsEndDateRelative eventend form-control" onchange="eventendat(this)";>
                  <option value="0">at:</option>
                  <option  value="1">certain time after the event begins</option>
                  <option value="2">certain time before the event begins</option>
                </select>
<span class="text-danger error-text SaleEnd_error"></span>
                <span class="cIsEndDateRelative0" style="display:inline-block;width:60% ">
                <input  type="date" 
                value="{{date('Y-m-d', strtotime($event_data->event_sales->end_date))}}" 
                name="EndDate" 
                maxlength="10" id="ctl02_cEndDateView" 
                class="cDatePicker cEndDateView hasDatepicker form-control " style="display:inline-block;width:30% ">

 
                <input  type="time" 
                name="EndTime" 
                value="{{date('H:i:s', strtotime($event_data->event_sales->end_time))}}" 
                maxlength="10" id="ctl02_cEndTimeView"
                 class="cTimePicker cEndTimeView hasTimepicker form-control" data-validation-required="1" data-validation-avoid-success-tick="1"
                  style="display:inline-block;width:30% ">
                ...
                <span class="cIsEndDateRelative1 cIsEndDateRelative2"  
                style="display: none;" >
...
<div class="row delivery" style="padding-left:220px;">
                <div class="row2" style="text-align:center;">
                  <a href="#custom-tabs-one-profile" data-toggle="pill"   class="btn btn-secondary nxtbtn add_event_next_button"><span>Step 2 Delivery & Returns</span>
                                      &nbsp;&nbsp; <i class="fa-angle-double-right fa fa-15x"></i></a>
 ...
 <br>
  ...
  <a  class="dltbtn btn btn-success  btn-sm" onclick="clear_form_elements('basic_info');" id="DataTrigger_info"  style="margin-left: 5px;">Delete</a>
	</div>
	</form>
</div>
